function status = test_backward(net, num_iters, is_num_dets_fixed, eps)
...
for j = 1:num_iters
    ...
    top_diff = single(rand(unary_dims(1), unary_dims(2), 1, num_dets+1));
    ...
    expected_diff = single(zeros(unary_dims(1), unary_dims(2), label_space, num_dets + 1));
    for k = 1:num_dets
        det_label = detections(1, 1, k);
        expected_diff(:, :, det_label+1, k+1) = top_diff(:, :, 1, k+1);
    end
    
    expected_diff(:, :, 1, 1) = top_diff(:, :, 1, 1);
