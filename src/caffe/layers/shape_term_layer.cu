...
namespace caffe{
  ...
  void ShapeTermLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
    ...
    for (int i = 0; i < detection_box_list_.size(); ++i){
      ...
      for (int j = 0; j < warped_shapes_.size(); ++j){
        ...
        const Dtype* shapes = warped_shapes_[j]->gpu_data();
        caffe_gpu_set(N, Dtype(1), ones.mutable_gpu_data());

        for (int k = 0; k < warped_shapes_[j]->channels(); ++k){

          caffe_gpu_mul(N, shapes + N*k, class_unary_rois_[i]->gpu_data(), temp.mutable_gpu_data());
          Dtype numerator;
          caffe_gpu_dot(N, ones.gpu_data(), temp.gpu_data(), &numerator);

          // For the denominator, we need to work out the norm
          caffe_gpu_mul(N, shapes + N*k, shapes + N*k, temp.mutable_gpu_data());
          Dtype sum_sq_norm;
          caffe_gpu_dot(N, ones.gpu_data(), temp.gpu_data(), &sum_sq_norm);
      ...
      const Dtype* matched_shape_ptr = warped_shapes_[a_index]->gpu_data();
      const int N = warped_shapes_[a_index]->height() * warped_shapes_[a_index]->width();
      matched_shape_ptr = matched_shape_ptr + b_index * N;
      caffe_gpu_copy(N, matched_shape_ptr, matched_shapes_[i]->mutable_gpu_data());
    ...
    caffe_gpu_set(zero_.count(), Dtype(0), zero_.mutable_gpu_data());
   ...
   */
  template<typename Dtype>
  void ShapeTermLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top, const vector<bool> &propagate_down,
  const vector<Blob<Dtype> *> &bottom) {

    Blob<Dtype>* unary_blob = bottom[0];
    caffe_gpu_set(unary_blob->count(), Dtype(0), unary_blob->mutable_gpu_diff());
    ...
    for (int i = 0; i < detection_box_list_.size(); ++i){
      ...
      Dtype* shape_grad_ptr = this->blobs_[shape_a_index]->mutable_gpu_diff();
      shape_grad_ptr = shape_grad_ptr + shape_b_index * N;

      caffe_gpu_add(N, shape_param_bottom_blob.gpu_diff(), shape_grad_ptr, shape_grad_ptr);
    }

  } // Backward_cpu


INSTANTIATE_LAYER_GPU_FUNCS(ShapeTermLayer);
