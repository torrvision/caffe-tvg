...
#include "caffe/layers/image_label_data_class_layer.hpp"
...
template <typename Dtype>
ImageLabelDataClassLayer<Dtype>::ImageLabelDataClassLayer(
    const LayerParameter &param) : BasePrefetchingDataClassLayer<Dtype>(param) {
...
template <typename Dtype>
void ImageLabelDataClassLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  */
   ...
   */
  this->label_space_ = this->layer_param_.image_label_data_param().label_space();
  if (this->layer_param_.image_label_data_param().ignore_first_channel_for_classification()) {
    this->label_space_ += -1;
  }
  vector<int> class_label_shape(4);
  class_label_shape[0] = batch_size; 
  class_label_shape[1] = 1; // only supports first channel of the label blob
  class_label_shape[2] = 1; 
  class_label_shape[3] = this->label_space_; 
  top[2]->Reshape(class_label_shape);

  for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
    this->prefetch_[i].data_.Reshape(data_shape);
    this->prefetch_[i].label_.Reshape(label_shape);
    this->prefetch_[i].class_label_.Reshape(class_label_shape);
  }
...
template <typename Dtype>
void ImageLabelDataClassLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void ImageLabelDataClassLayer<Dtype>::SampleScale(cv::Mat *image, cv::Mat *label) {
...
template <typename Dtype>
void ImageLabelDataClassLayer<Dtype>::ApplySampleRotation(cv::Mat & image, cv::Mat & label){
...
template <typename Dtype>
void ImageLabelDataClassLayer<Dtype>::ApplySampleGaussianBlur(cv::Mat &image){
...
template <typename Dtype>
void ImageLabelDataClassLayer<Dtype>::load_batch(BatchWithClassLabel<Dtype>* batch) {
  ...
  class_label_shape[2] = 1; 
  class_label_shape[3] = this->label_space_;
  batch->class_label_.Reshape(class_label_shape);

  Dtype* prefetch_data = batch->data_.mutable_cpu_data();
  Dtype* prefetch_label = batch->label_.mutable_cpu_data();
  // Dtype* prefetch_class_label = batch->class_label_.mutable_cpu_data();
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (this->layer_param_.image_label_data_param().make_transform_independent_classification_gt()) {
      MakeClassLabel(cv_label, batch->class_label_.mutable_cpu_data() + item_id * this->label_space_);
    }
    ...
    if (!this->layer_param_.image_label_data_param().make_transform_independent_classification_gt() && item_id == batch_size - 1) {
      MakeClassLabel(batch->label_, batch->class_label_);
    }
...
template <typename Dtype>
void ImageLabelDataClassLayer<Dtype>::MakeClassLabel(const Blob<Dtype> &label, Blob<Dtype> &class_label) {
  const Dtype* label_data_ = label.cpu_data();
  Dtype* class_label_data_ = class_label.mutable_cpu_data();
  caffe_set(class_label.count(), Dtype(0), class_label_data_);
  int cur_label;
  const int batch_size = this->layer_param_.image_label_data_param().batch_size();
  for (int j = 0; j < batch_size; j++) {
    for (int i = 0; i < label.count(2); i++) {
      // only goes through the first channel in the label blob
      cur_label = static_cast<int>(label_data_[i]);
      // if ignore_first_channel_for_classification flag is true, ignore label==0;
      if (this->layer_param_.image_label_data_param().ignore_first_channel_for_classification() && cur_label == 0) {
        continue;
      }
      int index_to_class_label = cur_label - this->layer_param_.image_label_data_param().ignore_first_channel_for_classification();
      // any number greater than the label_space param will be discarded
      if (index_to_class_label > this->label_space_ - 1) {
        continue;
      }
      // update the class label blob
      class_label_data_[index_to_class_label] = Dtype(1);
    }
    label_data_ += label.count(1);
    class_label_data_ += class_label.count(1);
  }
}

template <typename Dtype>
void ImageLabelDataClassLayer<Dtype>::MakeClassLabel(const cv::Mat& cv_label, Dtype* class_label_data_) {
  caffe_set(this->label_space_, Dtype(0), class_label_data_);
  int cur_label;
  for (int y = 0; y < cv_label.rows; ++y) {
    // assumes the data type stored in cv_label is 8-bit unsigned char
    const uchar* row_ptr = cv_label.ptr<uchar>(y);
    for (int x = 0; x < cv_label.cols; ++x) {
      cur_label = static_cast<int>(row_ptr[x]);
      // if ignore_first_channel_for_classification flag is true, ignore label==0;
      if (this->layer_param_.image_label_data_param().ignore_first_channel_for_classification() && cur_label == 0) {
        continue;
      }
...
INSTANTIATE_CLASS(ImageLabelDataClassLayer);
REGISTER_LAYER_CLASS(ImageLabelDataClass);
