...
#include "caffe/layers/image_label_data_index_layer.hpp"
...
template <typename Dtype>
ImageLabelDataIndexLayer<Dtype>::ImageLabelDataIndexLayer(
    const LayerParameter &param) : BasePrefetchingDataIndexLayer<Dtype>(param) {
...
template <typename Dtype>
void ImageLabelDataIndexLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ImageLabelDataIndexLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void ImageLabelDataIndexLayer<Dtype>::SampleScale(cv::Mat *image, cv::Mat *label) {
...
template <typename Dtype>
void ImageLabelDataIndexLayer<Dtype>::ApplySampleRotation(cv::Mat & image, cv::Mat & label){
...
template <typename Dtype>
void ImageLabelDataIndexLayer<Dtype>::ApplySampleGaussianBlur(cv::Mat &image){
...
template <typename Dtype>
void ImageLabelDataIndexLayer<Dtype>::load_batch(BatchWithIndex<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    prefetch_idx[item_id] = GetIndex(image_lines_[lines_id_]);
...
template <typename Dtype>
Dtype ImageLabelDataIndexLayer<Dtype>::GetIndex(const std::string & filename) {
  ...
  if ( this->layer_param_.image_label_data_param().filename_to_idx() ){
    ...
    std::string temp = filename.substr(slice_start, slice_end - slice_start + 1);
    std::string final = "";
...
INSTANTIATE_CLASS(ImageLabelDataIndexLayer);
REGISTER_LAYER_CLASS(ImageLabelDataIndex);
