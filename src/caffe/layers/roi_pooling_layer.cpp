...
template <typename Dtype>
void ROIPoolingLayer<Dtype>::set_pool_height_width(const int height, const int width, Dtype spatial_scale) {
  this->pooled_height_ = height;
  this->pooled_width_ = width;
  this->spatial_scale_ = spatial_scale;
}

template <typename Dtype>
void ROIPoolingLayer<Dtype>::transfer_pooling_switches(Blob<int> *to_copy_in) {
    to_copy_in->CopyFrom(this->max_idx_, false, true); // Don't copy diff, do reshape
}

template <typename Dtype>
void ROIPoolingLayer<Dtype>::set_pooling_switches(Blob<int> *to_copy_from) {
    this->max_idx_.CopyFrom(*to_copy_from, false, true);
}
